    ...
    return (
        <React.Fragment>
            <Modal
                ...
                closeOnDimmerClick open={modalOpen === 'email'}
                onClose={() => setModalOpen('email')}
            >
                <Modal.Header>
                    <Label color="blue" attached="top">Update Email</Label>
                    <Form>
                        <Form.Field>
                            <Input
                        ...
                        <Button negative onClick={() => setModalOpen(null)}>Cancel</Button>
                        <Button primary onClick={() => updateField('email', email)}>Save</Button>
                    </Form>
                </Modal.Header>
            </Modal>
            <Modal
                className="gm_field-change"
                closeOnDimmerClick open={modalOpen === 'password'}
                onClose={() => setModalOpen(null)}
            >
                <Modal.Header>
                    <Label color="blue" attached="top">Update Password</Label>
                    <Form>
                        <Form.Field>
                            <Input
                                maxLength={15}
                        ...
                        <Button negative onClick={() => setModalOpen(null)}>Cancel</Button>
                        <Button primary onClick={() => updateField('password', password)}>Save</Button>
                    </Form>
                </Modal.Header>
            </Modal>
            <Segment>
                        ...
                        {user.chars.map((char, i) => (
                            <Card key={`char_${i}`}>
                                <Card.Content>
                                    ...
                                    <Card.Description>
                                        <Icon name="power off" color={char.online ? 'green' : 'red'} /> {char.title}
                                    </Card.Description>
                                </Card.Content>
                        </Card>
                        ))}
                ...
                <div className="gm_profile-buttons">
                    ...
                    <Button floated="right" color="red" onClick={logout}>Logout</Button>
                </div>
            </Segment>
        </React.Fragment>
    );
