...
class Whosonline extends React.PureComponent {
    ...
    fetchChars({ limit, offset }) {
        ...
        apiUtil.get({ url: `/api/v1/chars?online=true&limit=${limit}&offset=${offset}`, json: true }, (error, data) => {
        ...
        return (
            <div className="gm_whosonline">
                <Segment.Group className="gm_online-count" raised>
                    <Segment><Icon name="power off" color={status === true ? 'green' : 'red'} /> Eden is {status === true ? 'online' : 'offline'}</Segment>
